...
if ($oldConn) {
    ...
    Write-Host "AVISO: Puerto 8765 ocupado (PID: $oldPid). Matando proceso..." -ForegroundColor Yellow
...
$bridge.Id | Out-File "$LogDir\bridge.pid" -Encoding ASCII
Write-Host "  OK Bridge PID: $($bridge.Id)" -ForegroundColor Green
...
$brain = Start-Process python -ArgumentList "`"$ScriptDir\local_brain_test.py`"" `
    ...
    -PassThru -WindowStyle Hidden
$brain.Id | Out-File "$LogDir\brain.pid" -Encoding ASCII
Write-Host "  OK Brain PID: $($brain.Id)" -ForegroundColor Green
Start-Sleep -Seconds 1
...
try {
    ...
    Write-Host "  AVISO: Bridge aun no responde. Revisa logs\bridge.log" -ForegroundColor Yellow
}
